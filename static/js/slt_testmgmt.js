...
$(document).ready( function() {
    ...
    // make default test config selection box as radio button
    ...
    // init RFID Card select box
    $("#uid_RfidCardCb").select2();
    // create selection change event handler
    $("#uid_RfidCardCb").on("change", function(e) {
        var rfid = $("#uid_RfidCardCb").val();
        if (!rfid)
            return;
        ...
        var href =  "/file?Action=DownloadTestConfig"
                  + "&Rfid=" + rfid
                  + "&Mode=All";
        $("a#uid_ExportAllTestCfg").attr({"href": href});

        get_board_config(rfid);
    });
    // get rfid number from URL & trigger combo box selection
    var rfid = get_param_by_name("Rfid");
    if (rfid)
        $("#uid_RfidCardCb").val(rfid).trigger("change");
    ...
    $("#uid_ImportAllTestCfg").click(function() {
        var rfid = $("#uid_RfidCardCb").val();
        if (!rfid) {
            $("#uid_MsgDlg").data("Message", "Please select User");
        ...
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            ...
            input_alltestcfg.onchange = function(e) {
                if (!e || e.target.files.length <= 0)
                    return;
                import_all_testcfg(rfid, e.target.files);
            };
    ...
    $("#uid_BoardCfgTbl").on("click", "#uid_InsertBoardCfg", function() {
        ...
        if (!Rfid) {
            ...
            var NewTestCfgDlg = $("#uid_NewTestCfgDlg").dialog({
                height        : 'auto',
                width         : 'auto',
                show          : 'fade',
                autoOpen      : false,
                ...
                buttons : {
                    "Insert" : function() {
                        var DefaultTestCfg_List = document.getElementsByName("NewTestCfg");
                        var Default_BoardCfg = null;
                        var Default_TestCfg = null;
                        for (var i = 0; i < DefaultTestCfg_List.length; ++i) {
                            var CheckedTest = DefaultTestCfg_List[i];
                            if (CheckedTest.checked == true && CheckedTest.getAttribute("data-id")) {
                                Default_BoardCfg = {
                                    'ID': null,
                                    'IniData': CheckedTest.getAttribute("data-ini"),
                                    'Text': CheckedTest.getAttribute("data-text"),
                                };
                                Default_TestCfg = CheckedTest.getAttribute("data-testcfg").replace(/'/g, '"');;
                                break;
                            }
                        }
                        ...
                        if (Default_BoardCfg)
                            commit_json_data(URL = "/test",
                                             Data = { Action: 'GenerateTestId', Rfid: Rfid },
                                             Param = {},
                                             OnSuccessCallback = function(json_resp, Param) {
                                                var resp = JSON.parse(JSON.stringify(json_resp));
                                                Default_BoardCfg.ID = resp["Data"];
                                                bind_board_cfg(Default_BoardCfg);
                                                commit_all_board_ini();
                                                commit_json_data(URL = "/test",
                                                                 Data = { Action: 'SetTestConfig',
                                                                          Rfid: Rfid,
                                                                          Data: Default_TestCfg,
                                                                          Arch: get_ini_param(Default_BoardCfg.IniData, "BOARD_CONFIG", "Arch") ,
                                                                          Test: Default_BoardCfg.ID
                                                                        });
                                            });
                        else
                            bind_board_cfg();
            ...
            NewTestCfgDlg.dialog("option", "title", "Insert new test");
            NewTestCfgDlg.dialog('open');
        }
        return false;
    });
        ...
        var delete_dialog = $("#uid_DeleteTestCfgDlg").dialog({
            ...
            buttons: {
                "Delete": function() {
                    ...
                    commit_all_board_ini(success_callback = function ( json_resp, Param ) {
                        var resp = JSON.parse(JSON.stringify(json_resp));
                        if (resp.Errno == 0) {
    ...
    dom_boardini_dlg.onchange = function(e) {
        ...
        if (!e || e.target.files.length <= 0)
            return;
        import_board_ini(e.target.files[0]);
    };
            ...
            "Update" : function() {
                ...
                var BoardCfg_Data = $("#uid_TestCfgTbl > tbody tr").map(function() {
                    return {
                        ...
                        Enable     : $(this).find("input[name='i_enable']").prop("checked") ? 1 : 0
                    }
                }).get();
    ...
    $("#uid_BoardCfgTbl").on("click", ".ucl_EditBoardCfg", function() {
        ...
        commit_json_data(URL = "/test",
                         Data = { Action : "GetTestConfig", Rfid: Rfid, Test: TestId, Arch: ArchId },
                         Param = {},
                         OnSuccessCallback = function ( json_resp, Param ) {
                            var resp = JSON.parse(JSON.stringify(json_resp));
                            if (resp.Errno == 0) {
    ...
    dom_maincsv_dlg.onchange = function(e) {
        ...
        if (!e || e.target.files.length <= 0)
            return;
        import_testcmd_main(e.target.files[0]);
    };
    ...
    dom_secondcsv_dlg.onchange = function(e) {
        ...
        if (!e || e.target.files.length <= 0)
            return;
        import_testcmd_second(e.target.files[0]);
    };
    ...
    dom_error1_dlg.onchange = function(e) {
        ...
        if (!e || e.target.files.length <= 0)
            return;
        import_error_1(e.target.files[0]);
    };
    ...
    dom_error2_dlg.onchange = function(e) {
        ...
        if (!e || e.target.files.length <= 0)
            return;
        import_error_2(e.target.files[0]);
    };
...
function commit_all_board_ini ( success_callback = null, error_callback = null ) {
    var Rfid = $("#uid_RfidCardCb").val();
    ...
    // commit
    commit_json_data(URL = "/test",
                     Data = { Action: 'SetBoardConfigList',
                              Rfid: Rfid,
...
function import_testcmd_main ( csv_file ) {
...
function import_testcmd_second ( csv_file ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    // read & parse csv file
    var file_reader = new FileReader();
...
function import_error_1 ( csv_file ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    // read & parse csv file
    var file_reader = new FileReader();
...
function import_error_2 ( csv_file ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    // read & parse csv file
    var file_reader = new FileReader();
...
function get_board_config ( Rfid ) {
    ...
    commit_json_data(URL = "/test",
                     Data = { Action : "GetBoardConfigList", Rfid: Rfid },
                     Param = {},
                     OnSuccessCallback = function ( json_resp, Param ) {
                        var resp = JSON.parse(JSON.stringify(json_resp));
                        if (resp.Errno) {
                            ...
                            // $("#uid_MsgDlg").data('Message', "Config loaded successfully");
                            // $("#uid_MsgDlg").dialog("open");
                        }
                     });
}
...
function bind_test_cfg ( testcfg = null ) {
    ...
    tr_html += '                <input type="checkbox" class="switch-input" id="i_enable" name="i_enable"/>';
    ...
    if (testcfg) {
        ...
        new_tr.find('input[name="i_enable"]').prop("checked", testcfg.Enable == 1 ? true : false);
...
function import_testcfg ( TestCfgID, menu_dipslay, fds ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
...
function import_maindata ( BoardCfg, MainData_fn, SecondData_fn, ErrorData1_fn, ErrorData2_fn, fds, callback ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    // find main data file
    var fd = null;
    ...
    if (!fd) {
        // import second data
        import_seconddata(BoardCfg, SecondData_fn, ErrorData1_fn, ErrorData2_fn, fds, callback);
        return;
    }
...
function import_seconddata ( BoardCfg, SecondData_fn, ErrorData1_fn, ErrorData2_fn, fds, callback ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    // find main data file
    var fd = null;
    ...
    if (!fd) {
        // import error data 1
        import_errordata1(BoardCfg, ErrorData1_fn, ErrorData2_fn, fds, callback);
        return;
    }
...
function import_errordata1 ( BoardCfg, ErrorData1_fn, ErrorData2_fn, fds, callback ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
    ...
    if (!fd) {
        // import error data 2
        import_errordata2(BoardCfg, ErrorData2_fn, fds, callback);
        return;
    }
...
function import_errordata2 ( BoardCfg, ErrorData2_fn, fds, callback ) {
    // Predefined constants for string processing
    var CELL_DELIMITER = ",";
...
function import_all_testcfg ( rfid, fds ) {
