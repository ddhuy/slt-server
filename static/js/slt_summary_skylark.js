...
//==== search function
    ...
    function build_summary_list_by_cpuid ( search_results ) {
        ...
        for (var i in search_results) {
            ...
            for (var j in summary_list) {
                ...
                if (summary.CpuId == result['CpuId'] && summary.LotNum == result['LotNum']) {
    ...
    function bind_search_result_production ( search_results ) {
        ...
        for (var i in SummaryList) {
            ...
            div_cpu_hdr.setAttribute("style", "width: 1200px;");
            ...
            for (var j = 0; j < summary.Summary.length; ++j) {
                ...
                if (summary.Summary[j - 1] && row.CpuId == summary.Summary[j - 1].CpuId) {
                    td.innerHTML  =   '<td rowspan="1">' + row.TestName + '</td>'
                                    ...
                                    + '<td rowspan="1">' + row.SocketSerial + '</td>'
                ...
                td.innerHTML += '<td rowspan="1" style="text-align:center;">\
                                    <a class="s_testdetail" href="#">Details</a> | \
                                    <a title="Download log file" href="file/?Action=DownloadLogFile&Rfid=' + row.Rfid + '&Filename=' + row.Log + '">Log</a> | \
                                    <a class="s_testhistory" href="" data-cpuid="' + row.CpuId + '" data-lotnum="' + row.LotNum + '">History</a>\
                                </td>';
        ...
        for (var i in SummaryList) {
            ...
            for (var j in summary.Summary) {
                ...
                html_tbl_row.innerHTML  =  '<td title="Test Name">' + row.TestName + '</td>'
                                         + '<td title="Bench number">' + row.BenchNum + '</td>'
                                         + '<td title="Board Serial">' + row.BoardId + '</td>'
